...
- **The guaranteed runnable ENTRY — the integration/assembly contract.** The module map + handles connect the divisions *to each other*, but nothing owned *the runnable product entry that wires them into an artifact that actually opens and runs.* A real 74-min run exposed the gap: the corp decomposed "a playable Snake game — ONE openable index.html" into **27 cross-importing TS modules + fragment HTML but NO root `index.html`, no entry that mounts the game** — "food with no home," which the tester gate (§8) flagged but could never fix. So the architect now **reserves a final integration entry** (`corp/architect.ts`), and — because a small model won't reliably author one — the plan **auto-injects a contract that owns it** (`corp/integration-contract.ts` `ensureIntegrationContract`, called in `run.ts` right before `buildOrgChartQueue`). For a web/renderable product it appends a synthesized `Contract` whose `slot` is the runnable entry (a root `index.html`, else a wiring `src/main.ts`), that **`dependsOn` every division output** (so it runs LAST, wiring real produced code), owned by an integration engineer node and **dispatched + verified + bounced like any other contract.** A pure-logic product has no browser entry to own, so none is injected (that class has no tester gate).

- **The delivery constraint reaches the decomposition.** A vision can carry a hard *delivery* shape — "ONE index.html that **opens directly in a browser without Node.js/npm/build**" — that must reach the architect, or it will happily pick a **bundler-dependent module graph that can never open directly** (the second half of the same real defect). `corp/delivery.ts` (`deriveDeliveryShape`) extracts a light, robust signal from the vision *text* (openable / no-build / single-file / web — never a hardcoded "snake"/"index.html") and threads it into both the **architect prompt** and the **integration contract's brief**, steering them toward a **self-contained openable entry** (inline / `<script type="module">` with relative paths / an import map — something a browser opens directly), not a build-only graph.
...
- **The tester gate PRODUCES the missing entry, it doesn't just flag it.** The review-at-merge **tester** lens builds/runs/screenshots the product; a renderable product with **no runnable entry** ("no home") is a blocking gate failure (`review.ts` `deriveTesterGateBlocking` / `hasRunnableEntry`), and the CEO's `APPROVE` is gated on it. But that failure maps to **no existing contract** — the entry is a *new* deliverable — so the ordinary re-dispatch bounce (which only re-runs existing contracts) could DETECT it but never FIX it: it flagged forever. The review phase now carries an integration-recovery seam (`review.ts` `DispatchIntegrationContractFn`, wired in `run.ts` exactly like `reviseForFindings`): when the entry is missing and maps to no contract, it **synthesizes + dispatches a new integration contract** (`buildIntegrationContract`, pruned deps + shared workspace so it reads the produced tree), **re-assembles the manifest, and recomputes the tester gate against it** — so a genuine fix clears the gate instead of dead-ending. It reuses the same `maxRevisions` / `RunBudget` net (bounded, never a deadlock, never throws). Belt-and-suspenders with §5's up-front auto-injection: normally the entry exists from the start; this recovery is the net for when it was *skipped* (e.g. a module it depended on failed).
